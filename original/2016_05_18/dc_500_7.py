...
def decompose(inXd, inYd, inZd, xmind, xmaxd, ymind, ymaxd, zmind, zmaxd, mnpd, xybufd, zbufd):    # inXd: list of x-coordinates \ inYd: list of y-coordinates \ inZd: list of z-coordinates
    ...
    xC = len(np.arange(xminP, xmaxP, xRes))             # for all possible x-coordinates that are within the subdomain (according to xy resolution) 
    yC = len(np.arange(yminP, ymaxP, yRes))         # for all possible y-coordinates that are within the subdomain (according to xy resolution)
    ...
    if len(inXd) is 0:    # if there are no data points or no regular grid points within subdomain, pass
        ...
        decompose(sdXYZ[3], sdXYZ[4], sdXYZ[5], sdXYZ[-3], xmaxd, ymind, sdXYZ[-2], zmind, sdXYZ[-1], mnpd, xybufd, zbufd)      # recursive function call 2
        decompose(sdXYZ[6], sdXYZ[7], sdXYZ[8], xmind, sdXYZ[-3], sdXYZ[-2], ymaxd, zmind, sdXYZ[-1], mnpd, xybufd, zbufd)      # recursive function call 3
        decompose(sdXYZ[9], sdXYZ[10], sdXYZ[11], sdXYZ[-3], xmaxd, sdXYZ[-2], ymaxd, zmind, sdXYZ[-1], mnpd, xybufd, zbufd)    # recursive function call 4
        decompose(sdXYZ[12], sdXYZ[13], sdXYZ[14], xmind, sdXYZ[-3], ymind, sdXYZ[-2], sdXYZ[-1], zmaxd, mnpd, xybufd, zbufd)   # recursive function call 5
        decompose(sdXYZ[15], sdXYZ[16], sdXYZ[17], sdXYZ[-3], xmaxd, ymind, sdXYZ[-2], sdXYZ[-1], zmaxd, mnpd, xybufd, zbufd)   # recursive function call 6
        decompose(sdXYZ[18], sdXYZ[19], sdXYZ[20], xmind, sdXYZ[-3], sdXYZ[-2], ymaxd, sdXYZ[-1], zmaxd, mnpd, xybufd, zbufd)   # recursive function call 7
        decompose(sdXYZ[21], sdXYZ[22], sdXYZ[23], sdXYZ[-3], xmaxd, sdXYZ[-2], ymaxd, sdXYZ[-1], zmaxd, mnpd, xybufd, zbufd)   # recursive function call 8
...
decompose(inX, inY, inZ, xmin, xmax, ymin, ymax, zmin, zmax, 50, int(xyB), int(zB))

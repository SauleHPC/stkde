#ifndef POINT_BASED_SYMOMP_POINT_HPP
#define POINT_BASED_SYMOMP_POINT_HPP
...
#pragma omp parallel reduction(+:eval)
  {
    ...
    ptemp[tid] = std::make_shared<util::Compact3D<values>>(voxX, voxY, voxT);
    
    util::Compact3D<values>& co = *(ptemp[tid]);

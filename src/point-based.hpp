#ifndef POINT_BASED_HPP
#define POINT_BASED_HPP
...
std::shared_ptr<util::Compact3D<stkde::values>> stkde_pointbased(const stkde::bounding_box& bb,
  ...
  //fill in zeroes
  // for (stkde::index i=0; i< co.getSizeX(); ++i)
  //   for (stkde::index j=0; j< co.getSizeY(); ++j)
  //     for (stkde::index k=0; k< co.getSizeZ(); ++k)
  // 	co(i,j,k) = 0;
  co.zero();
  ...
  for (stkde::index ob=0; ob<inst.obsx.size(); ++ob) {
	...
	for (stkde::index k = std::max(obsvt - voxtbw, (stkde::index)0); k< std::min(obsvt + voxtbw+1, voxT); ++k) {
	  ...
	  if (std::abs(vox_t - ot) <= pa.tbw
	    ...
	    stkde::values val = densityF(ox, oy, ot,
				  vox_x, vox_y, vox_t,

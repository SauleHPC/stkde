...
namespace stkde{
  dp_hier_val partition_jaggedZ_over_rec(dp_jagged_parameters& param,
	...
	//base case is one box
	{
	  ...
	  right_cut.sumload = ((right_cut.maxload <= maxloadallowed)?right_cut.maxload : std::numeric_limits<double>::infinity());
	}
  ...
  dp_hier_val partition_jaggedZ_over(dp_jagged_parameters& param,
				     const stkde::instance& inst,
				     index Xmin,
				     index Xmax,
				     index Ymin,
  ...
  dp_hier_val partition_jaggedY_over_rec(dp_jagged_parameters& param,
	...
	auto right_cut = partition_jaggedZ_over(param, inst, Xmin, Xmax, cut, Ymax, nbparts-p, maxloadallowed);
    ...
    //no ycut solution
    {
      auto right_cut = partition_jaggedZ_over(param, inst, Xmin, Xmax, 0, Ymax, nbparts, maxloadallowed);
  ...
  dp_hier_val partition_jaggedY_over(dp_jagged_parameters& param,
				     const stkde::instance& inst,
				     index Xmin,
				     index Xmax,
				     int nbparts,
  ...
  dp_hier_val partition_jaggedX_over_rec(dp_jagged_parameters& param,
	...
	auto right_cut = partition_jaggedY_over(param, inst, cut, X, nbparts-p, maxloadallowed);

	if (!potential(right_cut))
	  continue;

	if (!remain_is_small(voxelbox(0,X,0,c.voxY,0,c.voxT), p))
    ...
    //no xcut solution
    {
	auto right_cut = partition_jaggedY_over(param, inst, 0, X, nbparts, maxloadallowed);

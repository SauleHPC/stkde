#ifndef POINT_BASED_SYMOMP_OBSDECOMP_COLORSCHED_FROMFILE_HPP
#define POINT_BASED_SYMOMP_OBSDECOMP_COLORSCHED_FROMFILE_HPP
...
std::shared_ptr<util::Compact3D<values>> stkde_pointbased_symomp_obsdecomp_colorsched_fromfile(const bounding_box& bb,
										      const instance& inst,
										      const parameters& pa,
										      index decompsizeX,
										      index decompsizeY,
  ...
  if (0)
  {
    ...
    std::ifstream colormap (getenv("COLORMAP"));
    std::vector<std::pair<triplet, double>> ordering;
    while (colormap) {
      triplet loc;
      double value;
      colormap >>loc.x>>loc.y>>loc.z;
      colormap>> value;
      if (colormap) {
	ordering.push_back(std::make_pair(loc, value));
      }
    }
    std::sort(ordering.begin(), ordering.end(),
	      [](std::pair<triplet, double>& a, std::pair<triplet, double>& b) -> bool
	      {return a.second < b.second; }
	      );
    for (auto& p: ordering) {
      orderedblocks.push_back(p.first);
    }
  }
